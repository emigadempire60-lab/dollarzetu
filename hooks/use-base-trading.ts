...
export interface UseBaseTradingParams {
  ...
  /** Tick count history window (25, 50, 500, 1000). Defaults to 1000. */
  tickWindow?: number;
}
...
export function useBaseTrading({
  ...
  const { currentTick, prices, pipSize } = useTicks(ws, isConnected, activeSymbol, tickWindow);

  // Surface WS-level errors — buy and sell errors are handled by their own hooks
  useEffect(() => {
    // Background WS errors are handled by individual feature hooks
  }, [ws, isConnected]);
