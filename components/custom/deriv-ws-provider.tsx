...
import { useBalance, type BalanceInfo } from '@/hooks/use-balance';
...
export function DerivWSProvider({ children }: { children: React.ReactNode }) {
  ...
  const balanceInfo = useBalance(ws, isConnected, !!auth.wsUrl);

  return (
    <DerivWSContext.Provider value={{ ws, isConnected, isExhausted, auth, balanceInfo }}>
      {children}
